...
function calcCastFpa(frames: number, animSpeed: number, fcr: number, skillSlow: number): number {
...
export function computeBreakpointTable(
  ...
  const throwingPenalty = animType === 'A1' && params.isThrowing && animPrefix !== 'TH' ? 30 : 0;
